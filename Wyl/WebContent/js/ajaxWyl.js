...
function Service(services){	//一直到299行
	 ...
	 */
	this.appendServices=function(services){//到128行
		if(!services){
			return this;
		}
	 ...
	 * ...
	 */
	this.appendProc=function(procedures){		//如果传入的参数是空，null,''等，那么就返回该对象，实际上就是返回 new Service() 这个对象，@wyl
		if(!procedures){
			return this;
		}
		//默认按批量形式添加服务，如果是单个，那么用数组包装		//如果传入的不是数组，那么就把这个对象包装成数组，@wyl,并且用临时变量存起来
