...
<#if ((v_rows_arr?size)>0)>
    <tr fweditgrid-row-index=${irow} > 
       <#if bFixed>
         <td class=fweditgrid-td-rownumber><div class=fweditgrid-cell-rownumber>${irow+1}</div></td>
...
<div class=fweditgrid-header >
    <div class=fweditgrid-header-inner >
        <table  border=0 cellspacing=0 cellpadding=0>
            <tbody>
            <#assign td_rownumber>
                <td rowspan=${v_head_arr?size}><div class=fweditgrid-header-rownumber></div></td> 
                 <#if parameters.showDelete=="true" >
                  <td rowspan=${v_head_arr?size}><div class=fweditgrid-header-opr>操作</div></td> 
                 </#if>
            </#assign>
            ...
            <tr>
                ...
                <#list row as col><#--列数据生成 -->
                    <td rowspan=${col.rowspan!1} colspan=${col.colspan!1}>
                    <#if !(col.lastRowIndex??)><#--标题组生成方式 -->
                        <#if col.checkbox??><#--生成checkbox -->
                           <div class=fweditgrid-cell <#if col.width??>width=${col.width}</#if> > 
                            <input type=checkbox class=fweditgrid-header-check>
                           </div>
                        <#else>
                           <div class=fweditgrid-cell-group <#if col.width??>width=${col.width}</#if> >${col.title!""}</div>
                        </#if>
                    <#else><#--非标题组生成方式 -->
                        <div class=fweditgrid-cell <#if col.width??>style="width=${col.width}"</#if> lastRowIndex="${col.lastRowIndex}">
                            <#if col.checkbox??><#--生成checkbox -->
            ...
            <div class="fweditgrid-wrap">   <!-- 整个grid的覆盖区 --> 
                ...
                <div class="fweditgrid-view"> <!-- 整个grid的数据区 -->
                  ...
                  <div class=fweditgrid-body>
                     <div class=fweditgrid-body-inner>
                         <table class="gltDataTable" border=0 cellspacing=0 cellpadding=0>
