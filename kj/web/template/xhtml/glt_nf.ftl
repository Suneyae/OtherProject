...
<div class=fwdatagrid-header >
    <div class=fwdatagrid-header-inner >
            ...
            <#if ((v_head_arr?size) > 0 )>
                ...
                <tr>
                    ...
                    <#list row as col><#--列数据生成 -->
                        <td rowspan=${col.rowspan!1} colspan=${col.colspan!1} <#if col.align??>
                        _al=${col.align}</#if> sortable=${col.sortable!'true'}>
                        <#if !(col.lastRowIndex??)><#--标题组生成方式 -->
                        
                           <#if col.checkbox??><#--生成checkbox -->
                               <div class=fwdatagrid-cell <#if col.width??>width=${col.width}</#if> > 
                                <input type=checkbox class=fwdatagrid-header-check>
                            <#else>
                            <div class=fwdatagrid-cell-group <#if col.width??>width=${col.width}</#if> >
                                ${col.title!""}
                            </#if>
                            </div>
                        <#else><#--非标题组生成方式 -->
                            <div class=fwdatagrid-cell <#if col.width??>style="width=${col.width}"</#if> lastRowIndex="${col.lastRowIndex}">
                            <#if col.checkbox??><#--生成checkbox -->
...
<div class=fwdatagrid-body>
     <div class=fwdatagrid-body-inner></div>
</div>
</#macro>
<#-- 开始生成多记录表脚本-->
