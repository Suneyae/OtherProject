 ...
 public final class SysConfCache
   implements ICache<CacheItem<HashMap<String, String>>>
 {
   private static final Logger LOGGER = LoggerFactory.getLogger(SysConfCache.class);
   private static final long serialVersionUID = -7978348788757125969L;
   private static ICache<CacheItem<HashMap<String, String>>> oCache = new SysConfCache();
   private static HashMap<String, CacheItem<HashMap<String, String>>> cache;
   
   private SysConfCache()
   {
     cache = new HashMap();
   }
     ...
     throws Exception
   {
     try
     {
       List ls = dao.select("FW_CONFIG.AA01_Q", null);
       for (Iterator rows = ls.iterator(); rows.hasNext();)
       {
         ...
         String key = (String)o.get("AAA001");
     ...
     throws Exception
   {
     ...
     try
     {
       List ls = dao.select("FW_CONFIG.AA01_Q", args);
       CacheItem<HashMap<String, String>> code = new CacheItem();
       for (Iterator rows = ls.iterator(); rows.hasNext();)
       {
         flag = true;
