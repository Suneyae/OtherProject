 ...
 public final class ExcelConfCache
   implements ICache<CacheItem<HashMap<String, String>>>
 {
   private static final Logger LOGGER = LoggerFactory.getLogger(ExcelConfCache.class);
   private static final long serialVersionUID = 5729319303880408972L;
   private static ICache<CacheItem<HashMap<String, String>>> oCache = new ExcelConfCache();
   private static HashMap<String, CacheItem<HashMap<String, String>>> cache;
   
   private ExcelConfCache()
   {
     cache = new HashMap();
   }
     ...
     throws Exception
   {
     try
     {
       List ls = dao.select("FW_CONFIG.FW_EXPIMP_EXCEL_Q", null);
     ...
     throws Exception
   {
     ...
     try
     {
       List ls = dao.select("FW_CONFIG.FW_EXPIMP_EXCEL_Q", args);
       CacheItem<HashMap<String, String>> code = new CacheItem();
       for (Iterator rows = ls.iterator(); rows.hasNext();)
       {
         flag = true;
