 ...
 public final class SqlConfCache
   implements ICache<CacheItem<HashMap<String, String>>>
 {
   private static final Logger LOGGER = LoggerFactory.getLogger(SqlConfCache.class);
   private static final long serialVersionUID = -6741645003941400525L;
   private static ICache<CacheItem<HashMap<String, String>>> oCache = new SqlConfCache();
   private static HashMap<String, CacheItem<HashMap<String, String>>> cache;
   
   private SqlConfCache()
   {
     cache = new HashMap();
   }
     ...
     throws Exception
   {
     try
     {
       List ls = dao.select("FW_CONFIG.FW_QUERYSQL_Q", null);
     ...
     throws Exception
   {
     ...
     try
     {
       List ls = dao.select("FW_CONFIG.FW_QUERYSQL_Q", args);
       CacheItem<HashMap<String, String>> code = new CacheItem();
       for (Iterator rows = ls.iterator(); rows.hasNext();)
       {
         flag = true;
